...
import { getLevelupEnabled, getLevelupChannel, isFeatureEnabled } from './settings.js';
...
import { aiConfigured, chatReply, imitate, writeDiary } from './ai.js';
import {
...
import { getAutoRole, getVerifyRole, getWelcomeMessage, getLeaveChannel, getLeaveMessage } from './settings.js';
import { startGtn, handleGtnGuess } from './gtn.js';
import { startGiveaway, enterGiveaway } from './giveaway.js';
...
import { addMessageXp, buildRankCard, buildLeaderboard } from './levels.js';
import {
  getBalance,
  claimDaily,
  addCoins,
...
client.on(Events.GuildMemberAdd, async (member) => {
  try {
    ...
    if (isFeatureEnabled(member.guild.id, 'autorole')) {
      const arId = getAutoRole(member.guild.id);
      if (arId && member.guild.roles.cache.has(arId)) await member.roles.add(arId).catch(() => {});
    }
